...
import { scoreMotor } from "./scoreMotor.ts";

export async function scorePerformanceFromIds(
	desempenhoId: string | null | undefined,
	...
	if (desempenhoId) {
		const desempenho = await desempenhoService.getDesempenhoById(desempenhoId);
		if (desempenho) {
			const vm = parseNumber(desempenho.velocidadeMaxima);
			const acc = parseNumber(desempenho.aceleracao0100);

			vmScore = vm != null ? scaleTo100(vm, REFERENCE_VALUES.velocidadeMaxima.min, REFERENCE_VALUES.velocidadeMaxima.max) : 50;
			accScore = acc != null ? invertScaleTo100(acc, REFERENCE_VALUES.aceleracao0100.min, REFERENCE_VALUES.aceleracao0100.max) : 50;
		}
	}

	// 2. Score Motor (async)
	const motorScore = await scoreMotor(motorId);

	// Weighted combination
	return vmScore * 0.4 + accScore * 0.3 + motorScore * 0.3;
}
